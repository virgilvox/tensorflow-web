...
import { QuantizationParameters } from '../tflite/quantization-parameters';
import { SparsityParameters } from '../tflite/sparsity-parameters';
import { TensorType } from '../tflite/tensor-type';
import { VariantSubType } from '../tflite/variant-sub-type';
...
static createVariantTensorsVector(builder:flatbuffers.Builder, data:flatbuffers.Offset[]):flatbuffers.Offset {
  builder.startVector(4, data.length, 4);
  for (let i = data.length - 1; i >= 0; i--) {
...
static addExternalBuffer(builder:flatbuffers.Builder, externalBuffer:number) {
  builder.addFieldInt32(10, externalBuffer, 0);
}
